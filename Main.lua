...
-- New Hitbox Extend System (Customizable)
                ...
                task.spawn(function()
                    while char.Parent do
                        if _G.HitboxExtendEnabled then
                            ...
                            if mesh and mesh:IsA("SpecialMesh") then mesh.Scale = Vector3.new(_G.HitboxExtendSize, _G.HitboxExtendSize, _G.HitboxExtendSize) end
                        else
                            -- Reset khi tat
                            if not IsArsenal then part.Size = Vector3.new(1, 1, 1); part.Transparency = 0 end
                        end
                        task.wait(0.5)
                    end
                end)
...
-- Tab RAGE (Added Auto Shoot & Extend Hitbox Settings)
...
addSlider("Shoot Delay", 0, 1, "AutoShootDelay", rPage, true)

if IsArsenal then 
    addToggle("Arsenal Hitbox", "HitboxEnabled", rPage); addSlider("HB Size", 2, 25, "HitboxSize", rPage)
else 
    addToggle("Hitbox Extend", "HitboxExtendEnabled", rPage)
    addSlider("HE Size", 2, 20, "HitboxExtendSize", rPage)
...
RunService.RenderStepped:Connect(function()
    Grad.Offset = Vector2.new(math.sin(tick() * 1.5) * 0.5, 0); Watermark.Visible = _G.WatermarkEnabled
    ...
    if _G.AimbotEnabled and IsTargeting then 
        ...
        if t then 
            ...
            if _G.AutoShootEnabled and tick() - lastShootTime >= _G.AutoShootDelay then
                mouse1click(); lastShootTime = tick()
            end
        end 
    end
end)
