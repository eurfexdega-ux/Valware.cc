                ...
                if onScreen then
                    ...
                    if mag < dist then
                        ...
                        if _G.AimbotWallCheck then
                            local hit = Camera:GetPartsObscuringTarget({p.Position}, {Player.Character, v.Character})
                            if #hit > 0 then pass = false end
                        end
                        if pass then target = p; dist = mag end
                    end
                end
            end
        end
...
local RGBLine = Instance.new("Frame", Main); RGBLine.Size = UDim2.new(1, 0, 0, 4); RGBLine.BorderSizePixel = 0
...
TabBar.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; dragStart = input.Position; startPos = Main.Position end end)
...
if IsArsenal then addToggle("Arsenal Hitbox", "HitboxEnabled", rPage) addSlider("HB Size", 2, 25, "HitboxSize", rPage) addSlider("HB Alpha", 0, 1, "HitboxTransparency", rPage, true) else addToggle("Hitbox Extend", "HitboxExtendEnabled", rPage) addSlider("HE Size", 2, 15, "HitboxExtendSize", rPage) addSlider("HE Alpha", 0, 1, "HitboxExtendTransparency", rPage, true) end
...
if IsCounterBlox then addToggle("Enable No-Spread", "NoSpreadEnabled", miPage, nil, function(v) if v then scan_for_weapons() apply_no_spread_all() else restore_spread_all() end end) end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    local mP = RaycastParams.new(); mP.FilterDescendantsInstances = {char}; mP.FilterType = Enum.RaycastFilterType.Exclude  
    if _G.JumpBugEnabled and UserInputService:IsKeyDown(_G.JumpBugKey ~= Enum.KeyCode.Unknown and _G.JumpBugKey or Enum.KeyCode.Space) then  
        local cast = workspace:Raycast(root.Position, Vector3.new(0, -hum.HipHeight - 2.1, 0), mP)
        if cast and root.Velocity.Y < 0 then hum:ChangeState(Enum.HumanoidStateType.Jumping) root.AssemblyLinearVelocity = Vector3.new(root.Velocity.X * 1.1, hum.JumpPower + 5, root.Velocity.Z * 1.1) notify("JB") end
    end  
    if _G.EdgeBugEnabled and UserInputService:IsKeyDown(_G.EdgeBugKey) then  
        local floor = false; for _, cp in pairs({Vector3.zero, Vector3.new(0.6,0,0), Vector3.new(-0.6,0,0), Vector3.new(0,0,0.6), Vector3.new(0,0,-0.6)}) do if workspace:Raycast(root.Position + cp, Vector3.new(0,-5,0), mP) then floor = true break end end
        if not floor and hum.FloorMaterial == Enum.Material.Air and root.Velocity.Y < -5 then root.AssemblyLinearVelocity = Vector3.new(root.Velocity.X, -0.001, root.Velocity.Z) notify("EB") end  
    end
    if _G.LongJumpEnabled and hum.FloorMaterial == Enum.Material.Air then  
        ...
        if diff > 0.02 then local add = Camera.CFrame.LookVector * (diff * 18 * (_G.FlickBoost or 1)); local vel = root.Velocity + Vector3.new(add.X, 0.5, add.Z) if Vector3.new(vel.X, 0, vel.Z).Magnitude > _G.MaxLongJumpSpeed then local flat = Vector3.new(vel.X, 0, vel.Z).Unit * _G.MaxLongJumpSpeed; root.Velocity = Vector3.new(flat.X, vel.Y, flat.Z) else root.Velocity = vel end if Vector3.new(root.Velocity.X, 0, root.Velocity.Z).Magnitude >= 30 then notify("LJ") end end LastCameraYaw = curYaw  
    end  
    if _G.PixelSurfEnabled and UserInputService:IsKeyDown(_G.PixelSurfKey) then  
        local ray = workspace:Raycast(root.Position, Camera.CFrame.LookVector * 5, mP)  
        if ray and hum.FloorMaterial == Enum.Material.Air then hum:ChangeState(Enum.HumanoidStateType.Freefall) SurfTimer = (SurfTimer or 0) + dt; if SurfTimer >= 1 then SurfSpeed = math.min((SurfSpeed or 20) + 1, 100) SurfTimer = 0 end; local dir = (Vector3.new(root.Velocity.X, 0, root.Velocity.Z) - ray.Normal * Vector3.new(root.Velocity.X, 0, root.Velocity.Z):Dot(ray.Normal)) root.Velocity = Vector3.new(dir.Unit.X * (SurfSpeed or 20), -0.01, dir.Unit.Z * (SurfSpeed or 20)) notify("PS") else SurfSpeed, SurfTimer = 20, 0 end  
    end  
    if _G.BhopEnabled and UserInputService:IsKeyDown(_G.BhopKey) and hum.FloorMaterial ~= Enum.Material.Air then hum.Jump = true end
    root.Anchored = (_G.AirStuckEnabled and UserInputService:IsKeyDown(_G.AirStuckKey))
end)

RunService.RenderStepped:Connect(function()
    local char = Player.Character; local root = char and char:FindFirstChild("HumanoidRootPart")
    for _, v in pairs(Players:GetPlayers()) do if v ~= Player and v.Character and v.Character:FindFirstChild("ValCharms") then v.Character.ValCharms.Enabled = _G.EspCharmsEnabled end end
    if _G.ThirdPersonEnabled and char and root then
        local start = root.Position + Vector3.new(0, 2, 0)
        local rot = Camera.CFrame - Camera.CFrame.Position
        local target = start - (rot * Vector3.new(0, 0, _G.ThirdPersonDist)).Position
        local ray = workspace:Raycast(start, (target - start), RaycastParams.new())
        Camera.CFrame = CFrame.new(ray and (ray.Position + ray.Normal * 0.5) or target, start)
...
UserInputService.InputBegan:Connect(function(i, g) if not g then if i.KeyCode == _G.MenuKey then Main.Visible = not Main.Visible end if i.KeyCode == _G.AimbotKey then IsTargeting = true end if i.KeyCode == _G.ThirdPersonKey then _G.ThirdPersonEnabled = not _G.ThirdPersonEnabled end end end)
