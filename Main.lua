...
RunService.Heartbeat:Connect(function(dt)
    ...
    if _G.JumpBugEnabled and UserInputService:IsKeyDown(_G.JumpBugKey ~= Enum.KeyCode.Unknown and _G.JumpBugKey or Enum.KeyCode.Space) then  
        ...
        if cast and root.Velocity.Y < 0 then
            ...
            root.AssemblyLinearVelocity = Vector3.new(root.Velocity.X * 1.1, (hum.JumpPower > 0 and hum.JumpPower or 50) + 5, root.Velocity.Z * 1.1)
            notify("JB")
        end
    end  
    ...
    if _G.LongJumpEnabled and hum.FloorMaterial == Enum.Material.Air then  
        ...
        if diff > 0.02 then  
            ...
            root.Velocity = root.Velocity + Vector3.new(addForce.X, 0.5, addForce.Z)  
        end  
