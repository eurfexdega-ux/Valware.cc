...
local function IsVisible(targetPart)
    ...
    local obs = Camera:GetPartsObscuringTarget(castPoints, {char, targetPart.Parent})
    return #obs == 0
end
        ...
        if IsArsenal and _G.HitboxEnabled then
            for _, v in pairs(Players:GetPlayers()) do
                if v ~= Player and v.Character then
                    ...
                    if root and (not _G.AimbotWallCheck or IsVisible(root)) then
        ...
        elseif not IsArsenal and _G.HitboxExtendEnabled then
            for _, v in pairs(Players:GetPlayers()) do
                if v ~= Player and v.Character then
                    ...
                    if head and head:IsA("BasePart") and (not _G.AimbotWallCheck or IsVisible(head)) then
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= Player and v.Character then
            ...
            if p and h and h.Health > 0 then
                if _G.AimbotWallCheck and not IsVisible(p) then continue end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if _G.JumpBugEnabled and UserInputService:IsKeyDown(_G.JumpBugKey ~= Enum.KeyCode.Unknown and _G.JumpBugKey or Enum.KeyCode.Space) then  
        ...
        if cast and root.Velocity.Y < 0 then
            ...
            local power = (hum.JumpPower > 0 and hum.JumpPower or 50) + 5
            root.AssemblyLinearVelocity = Vector3.new(hVel.X * 1.1, power, hVel.Z * 1.1)
            notify("JB")
        end
    end  
    ...
    if _G.EdgeBugEnabled and UserInputService:IsKeyDown(_G.EdgeBugKey) then  
        ...
        local checkPoints = {Vector3.new(0,0,0), Vector3.new(0.6,0,0), Vector3.new(-0.6,0,0), Vector3.new(0,0,0.6), Vector3.new(0,0,-0.6)}
        for _, cp in pairs(checkPoints) do
            if workspace:Raycast(root.Position + cp, Vector3.new(0, -5, 0), moveParams) then hasFloor = true break end
        end
        if not hasFloor and hum.FloorMaterial == Enum.Material.Air and root.Velocity.Y < -5 then  
...
addToggle("No Spread", "NoSpreadEnabled", miPage, nil, function()
    update_all_weapons()
end)
...
RunService.RenderStepped:Connect(function()
    ...
    -- Third Person Fixed Logic
    if _G.ThirdPersonEnabled and char and root then
        local startPos = root.Position + Vector3.new(0, 2, 0)
        local targetPos = startPos - (Camera.CFrame.LookVector * _G.ThirdPersonDist)
        local rayParams = RaycastParams.new()
        rayParams.FilterDescendantsInstances = {char}
        local ray = workspace:Raycast(startPos, (targetPos - startPos), rayParams)
        
        local finalPos = ray and ray.Position or targetPos
        Camera.CFrame = CFrame.new(finalPos, startPos)
