...
_G.JumpBugEnabled, _G.JumpBugKey = false, Enum.KeyCode.Space
...
-- No Spread System (Gắn mới)
local function apply_no_spread_all()
    for _, v in ipairs(game:GetDescendants()) do
        if v:IsA("NumberValue") or v:IsA("IntValue") then
            local name = v.Name:lower()
            if name:find("spread") or name:find("accuracy") then v.Value = 0 end
        end
    end
end
...
-- Hitbox Handling (Gắn mới - Skin Fix)
task.spawn(function()
...
-- UI Setup (Giữ nguyên cấu trúc của bạn)
...
addToggle("No Spread", "NoSpreadEnabled", miPage, nil, function(v) if v then apply_no_spread_all() end end)
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if _G.JumpBugEnabled and UserInputService:IsKeyDown(_G.JumpBugKey) then  
        ...
        if ray and root.Velocity.Y < 0 then 
            ...
            root.AssemblyLinearVelocity = Vector3.new(root.Velocity.X * 1.05, 42, root.Velocity.Z * 1.05) 
        end
    end  
    ...
    if _G.EdgeBugEnabled and UserInputService:IsKeyDown(_G.EdgeBugKey) then  
        if not workspace:Raycast(root.Position, Vector3.new(0, -5, 0), moveParams) and hum.FloorMaterial == Enum.Material.Air and root.Velocity.Y < -5 then  
            root.AssemblyLinearVelocity = Vector3.new(root.Velocity.X, -0.001, root.Velocity.Z)
        end  
    end
    ...
    if _G.PixelSurfEnabled and UserInputService:IsKeyDown(_G.PixelSurfKey) then  
        ...
        if ray and hum.FloorMaterial == Enum.Material.Air then root.Velocity = Vector3.new(root.Velocity.X, -0.01, root.Velocity.Z) end
    end  
...
RunService.RenderStepped:Connect(function()
    ...
    if _G.NoSpreadEnabled then apply_no_spread_all() end
end)
