...
-- Improved Jump Bug (High Fall Trigger)
if _G.JumpBugEnabled and UserInputService:IsKeyDown(_G.JumpBugKey ~= Enum.KeyCode.Unknown and _G.JumpBugKey or Enum.KeyCode.Space) then  
    local ray = workspace:Raycast(root.Position, Vector3.new(0, -hum.HipHeight - 2.5, 0), moveParams)
    -- Trigger khi đang rơi xuống (Y < 0) và sắp chạm đất
    if ray and root.Velocity.Y < -5 then
        hum:ChangeState(Enum.HumanoidStateType.Jumping)
        local hVel = Vector3.new(root.Velocity.X, 0, root.Velocity.Z)
        local jumpPower = (hum.JumpPower > 0 and hum.JumpPower or 50) + 5
        root.AssemblyLinearVelocity = Vector3.new(hVel.X, jumpPower, hVel.Z)
        notify("JB")
    end
end  
